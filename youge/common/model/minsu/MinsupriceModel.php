...
class  minsupriceModel extends CommonModel{
    protected $pk       = 'price_id';
    protected $table    = 'minsu_room_price';
    ...
    */
    public function backPrice($minsu_id,$miniapp_id,$date,$online = false){
        $room_where['minsu_id'] = $where['minsu_id'] = (int) $minsu_id;
        ...
        foreach ($room as $val){
              $rooms[$val->room_id] = [
                  //当天价格
                  'minsuprice_id' => empty($prices[$val->room_id]) ? 0 : $prices[$val->room_id]->price_id,
                  'room_id' => $val->room_id,
