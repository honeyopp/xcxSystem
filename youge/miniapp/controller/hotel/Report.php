<?php
 ...
 class Report extends Common{
     ...
     public function store(){
         ...
         $fangdong = HotelModel::field("FROM_UNIXTIME(add_time,'%Y-%m-%d') as day , count(*) as num  ")->where($where)->group('FROM_UNIXTIME(add_time,\'%Y-%m-%d\')')->select();
     ...
     public function order(){
         ...
         $where['member_miniapp_id'] = $this->miniapp_id;
         $where['add_time'] = array('between',array(strtotime($date['BingDate']),strtotime($date['EndDate']) + 86400));
         $fangdong = HotelorderModel::field("FROM_UNIXTIME(add_time,'%Y-%m-%d') as day , count(*) as num  ")->where($where)->group('FROM_UNIXTIME(add_time,\'%Y-%m-%d\')')->select();
         $data = $countModel->checkDate($date['day'],$fangdong);
         $this->assign('data',$data);
         $this->assign('search',$search);
         return $this->fetch();
