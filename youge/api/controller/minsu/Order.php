...
class Order extends Common {
    ...
    public function cancel(){
        ...
        if ($MinsuorderModel->save(['status'=>3,'cancel_type'=>1,'cancel_time'=>  $this->request->time(),'cancel_info'=>$cancel_info],['order_id'=>$id])){
            if(!empty($order->pay_coupon_id)){
                $CouponModel = new CouponModel();
                $CouponModel->save(['is_can'=>0],['coupon_id'=>$order->pay_coupon_id]);
            }

            $minsu = MinsuModel::get($order->minsu_id);
            //发送模版消息
            $formId = $this->request->param('formId');
            $Miniapp = new MiniApp($this->appid);
            $Miniapp->sendTemplateMessage('AT0012', $this->user->open_id, $formId,'/pages/minsu/order/detail?id='.$id,[
                $minsu->minsu_name,
                $id,
    ...
    public function getPayment(){
       ...
       // var_dump($setting);die;
        ...
        // $input->SetGoods_tag();
        $input->SetNotify_url("https://".$_SERVER['HTTP_HOST']."/api/weixin/notifyminsu/appid/".$this->appid.'.html');
    ...
    public function create() {
        ...
        if(!$minsu = $MinsuModel->get($room->minsu_id)){
            $this->result([], '400', '该酒店不存在了', 'json');
        }
        $prices = MinsupriceModel::where(['room_id' => $id, 'day' => ['BETWEEN', [$bg_date, $end_date]]])->select();
        $pricesData = [];
        ...
        if($MinsuorderModel->save($order)){
            ...
            $formId = $this->request->param('formId');
            $Miniapp = new MiniApp($this->appid);
            $Miniapp->sendTemplateMessage('AT0008', $this->user->open_id, $formId,'/pages/minsu/order/detail?id='.$MinsuorderModel->order_id,[
                $MinsuorderModel->order_id,
                round($needpay/100,2),
                '等待支付',
                date('Y-m-d H:i:s'),
                $minsu->minsu_name,
            ...
            $priceModel = new MinsupriceModel();
            $priceModel->saveAll($saveAll);

            $MinsuModel = new MinsuModel();
            $MinsuModel->IncDecCol($room->minsu_id,'order_num',1);
            $this->result(['id'=>$MinsuorderModel->order_id], '200', '创建订单成功', 'json');
        }
    ...
    public function geTorder() {
        ...
        $MinsuorderModel = new MinsuorderModel();
        $totalNum = $MinsuorderModel->where($where)->count();
        ...
        foreach ($list as $val) {
            $data['list'][] = [
                ...
                'room_id' => $val->room_id,
                'user_id' => $val->user_id,
                'minsu_name' => empty($minsus[$val->minsu_id]) ? '' : $minsus[$val->minsu_id]->minsu_name,
    ...
    public function comment() {
        ...
        switch ($data['score']) {
            case $data['score'] < 25;
                $MinsuModel->where('minsu_id', $order->minsu_id)->setInc('bad_num');
                break;
            case $data['score'] > 35;
                $MinsuModel->where('minsu_id', $order->minsu_id)->setInc('praise_num');
                break;
        }
        $MinsuorderModel->save(['is_comment' => 1], ['order_id' => $order_id]);
        $ordernum = $MinsuorderModel->where(['user_id' => $this->user->user_id, 'member_miniapp_id' => $this->appid])->count();
        //首单赠送红包；
        $coupon_data = [];
        if ($ordernum <= 1) {
    ...
    public function getLike(){
        $MinsuLikeModel = new MinsuLikeModel();
        ...
        $list = $MinsuLikeModel->where($where)->limit($this->limit_bg,$this->limit_num)->select();
        $minsuIds = [];
        foreach ($list as $val){
            $minsuIds[$val->minsu_id] = $val->minsu_id;
        }
        ...
        foreach ($list as $val) {
            $data['list'][] = [
                'like_id' => $val->like_id,
                'minsu_id' => $val->minsu_id,
                'juli'       => getDistance($lat, $lng, $minsus[$val->minsu_id]->lat, $minsus[$val->minsu_id]->lng),
                'photo'   => empty($minsus[$val->minsu_id]) ? '' :   IMG_URL . getImg($minsus[$val->minsu_id]->photo),
                'score'   => empty($minsus[$val->minsu_id]) ? '' :   sprintf("%.0f",$minsus[$val->minsu_id]->score/10),
                'price'   => empty($minsus[$val->minsu_id]) ? '' :   sprintf("%.2f",$minsus[$val->minsu_id]->price/100),
                'minsu_name' => empty($minsus[$val->minsu_id]) ? '' :   $minsus[$val->minsu_id]->minsu_name,
                'praise_num' => empty($minsus[$val->minsu_id]) ? '' :   $minsus[$val->minsu_id]->praise_num,
                'bad_num' => empty($minsus[$val->minsu_id]) ? '' :   $minsus[$val->minsu_id]->bad_num,
                'room_num' => empty($minsus[$val->minsu_id]) ? '' :   $minsus[$val->minsu_id]->room_num,
                'advantage' => empty($minsus[$val->minsu_id]) ? '' :   $minsus[$val->minsu_id]->advantage,
                'appropriate' => empty($minsus[$val->minsu_id]) ? '' :   $minsus[$val->minsu_id]->appropriate,
            ];
        }
        $data['more']  = count($data['list']) == $this->limit_num ? 1: 0;
        $this->result($data,'200','数据初始化成功','json');
    ...
    public function setLike(){
        ...
        $MinsuLikeModel= new MinsuLikeModel();
        if($like = $MinsuLikeModel->where($where)->find()){
            $MinsuLikeModel->where($where)->delete();
        }else{
            $MinsuLikeModel->save($where);
